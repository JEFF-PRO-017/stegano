...
def chiffre(file, dossier_int, fileOut):
    ...
    with open(fileOut + '/' + cache[1], 'ab') as elt1:
        # insererement le separarteur
        elt1.write('##'.encode('utf-8'))
        with open(file, 'rb') as elt2:
            # insererement du flux chriffer
...
def decrypt(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            client = 'media/client'
            out = 'media/out'
            chiffre(file, client, out)

            return render(request, 'audio_steg/decrypt.html', {'form': form, 'nameFile': file_to_crypt})
